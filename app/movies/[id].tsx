...
const MoviesInfo = ({label, value}: MovieInfoProps) => (
    ...
    return (
        <View className={"bg-primary flex-1"}>
            {loading ? (<ActivityIndicator size={"large"} color={"#0000ff"}
                                           className={"mt-20 self-center"}/>) : (<View>
                <ScrollView contentContainerStyle={{paddingBottom: 80}}>
                    ...
                    <View className={"flex-col items-start justify-center mt-5 px-5"}>
                        ...
                        <MoviesInfo label={"Overview"} value={movie?.overview}/>

                        <MoviesInfo label={"Genre"} value={movie?.genres?.map((g) => g.name).join(' - ') || 'N/A'}/>

                        <View className={"flex flex-row justify-between w-1/2"}>
                            <MoviesInfo label={"Budget"}
                                        value={typeof movie?.budget === 'number' && movie?.budget > 0
                        ...
                        <MoviesInfo label={"Production Companies"}
